...
// adjust import path to your email util if different:
...
export async function POST(req) {
  try {
    ...
    if (!userId || !toOrgId) {
      return NextResponse.json({ error: "userId and toOrgId required" }, { status: 400 });
    }

    const allowed = await allowedOrgIdsFor(session);
    if (!allowed.includes(Number(toOrgId))) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
    ...
    if (customRoleId) {
      ...
      if (!cr || cr.org_id !== Number(toOrgId)) {
    ...
    // best-effort email (does not fail the request)
    try {
